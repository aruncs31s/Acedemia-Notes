...
```mermaid
flowchart TD
    A[Received Signal Branches<br/>r1, r2, ..., rN] --> B[SNR Monitor]
    B --> C{Compare SNR of all branches}
    C --> D1[Branch 1 Selected]
    C --> D2[Branch 2 Selected]
    C --> DN[Branch N Selected]
    D1 --> G[Output Signal y = r1]
    D2 --> G
    DN --> G
...
- **Weight calculation**: w_i = h_i* divided by (|h_i|²) where h_i is channel coefficient
- **Output**: y = Σ w_i times r_i where r_i is received signal on i-th branch
...
```mermaid
flowchart TD
    A[Received Signal Branches<br/>r1, r2, ..., rN] --> B[Channel Estimation]
    B --> C[Calculate Weights wi]
    C --> D[Weighted Combination y]
    D --> E[Output Signal Maximum SNR]
